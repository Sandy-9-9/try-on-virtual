...
async function callLovableGateway(apiKey: string, modelImage: string, clothImage: string): Promise<TryOnResult> {
  ...
  // Try standard OpenAI-style image response first
  const directUrl = data.choices?.[0]?.message?.images?.[0]?.image_url?.url;
  if (directUrl && directUrl.startsWith("data:")) return { image: directUrl };

  // Try extracting from content parts (Gemini multimodal response)
  ...
  // Try inline_data parts from Gemini native format tunneled through gateway
